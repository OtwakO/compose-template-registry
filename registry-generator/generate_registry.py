...
class RegistryStruct:
    ...
    description: str = "Private Registry for Arcane for docker compose file management"
    version: str = "1.0.0"
    author: str = REPOSITORY_OWNER
    url: str = f"{GITHUB_SERVER_URL}/{REPOSITORY_NAME}"
            ...
            if compose_file := list(template_dir.glob("*compose.y*ml")):
                ...
                templates.append(
                    ComposeTemplate(
                        ...
                        compose_url=f"{GITHUB_SERVER_URL}/{REPOSITORY_NAME}/templates/{template_dir.name}/{compose_file[0].name}",
                        env_url=f"{GITHUB_SERVER_URL}/{REPOSITORY_NAME}/templates/{template_dir.name}/.env.example",
                        documentation_url=f"{GITHUB_SERVER_URL}/{REPOSITORY_NAME}/templates/{template_dir.name}/README.md",
                        tags=["app"],
                    )
                )
    return templates
